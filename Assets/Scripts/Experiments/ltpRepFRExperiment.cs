...
public class ltpRepFRRun {
  public StimWordList encoding;
  public StimWordList recall;

  public ltpRepFRRun(StimWordList encoding_list, StimWordList recall_list,
...
public class ltpRepFRSession : List<ltpRepFRRun> {
}


public class ltpRepFRExperiment : ExperimentBase {
  ...
  protected ltpRepFRSession currentSession;
  ...
  public ltpRepFRExperiment(InterfaceManager _manager) : base(_manager) {
    ...
    state.restLists = manager.GetSetting("restLists").ToObject<int[]>();
  ...
  protected void DoStartTrial() {
    ...
    else if(state.restLists.Contains(state.listIndex)) {
  ...
  public ltpRepFRSession LoadRepFRSession(string participant, int session) {
    if(System.IO.File.Exists(System.IO.Path.Combine(manager.fileManager.SessionPath(participant, session), "session_words.json"))) {
      ...
      return JsonConvert.DeserializeObject<ltpRepFRSession>(json);
    }
    else{
      return null;
    }
  ...
  public ltpRepFRRun MakeRun(RandomSubset subset_gen, bool enc_stim,
      bool rec_stim) {
    ...
    return new ltpRepFRRun(enclist, reclist, enc_stim, rec_stim);
  }


  public ltpRepFRSession GenerateSession() {
    ...
    var session = new ltpRepFRSession();
    ...
    var randomized_list = new ltpRepFRSession();
