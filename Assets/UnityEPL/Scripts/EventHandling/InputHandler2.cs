...
using InputHandlerFunc = System.Func<InputHandler2, KeyMsg, bool>;

public class InputHandler2 : MessageTreeNode<KeyMsg> {
    protected KeyMsg keyMsg;
    protected AutoResetEvent keyMsgWritten = null;
    protected EventLoop waitOnKeyEventLoop = null;

    public InputHandler2(EventQueue host, InputHandlerFunc action) {
        this.host = host;
        SetAction(action);
    ...
    private InputHandler2(EventQueue host) {
        this.host = host;
        SetAction(ReportKey);
    ...
    public virtual void SetAction(InputHandlerFunc action) {
        ...
        this.action = (node, msg) => action((InputHandler2)node, msg);
    }

    public static bool ReportKey(InputHandler2 handler, KeyMsg msg) {
        if (!msg.down) {
            return false;
        }
    ...
    public KeyMsg WaitOnKey(InterfaceManager im, bool turnOffAllHandlers = true) {
        if (turnOffAllHandlers) {
            ...
            var tempInputHandler = new InputHandler2(waitOnKeyEventLoop);

            // Replace im input handler
            var priorImInputHandler = im.DoGet(new Task<InputHandler2>(() => {
                var priorInputHandler = im.inputHandler2;
                im.inputHandler2 = tempInputHandler;
            ...
            im.DoBlocking(new EventBase(() => im.inputHandler2 = priorImInputHandler));
