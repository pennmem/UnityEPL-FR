...
public class InterfaceManager2 : MonoBehaviour 
{
    ...
    //////////
    private YieldedEventQueue mainEvents = new EventQueue();
    ...
    public StateRet ShowVideo(string video, bool skippable) {
        DoBlocking(ShowVideoHelper(video, skippable, () => {
            
        }));
    }

    public StateRet ShowVideoHelper(string video, bool skippable, Action callback) {
        ...
        videoControl.StartVideo(videoPath, skippable, () => {
            resetEvent.Set();
        });
    ...
    public void Do(IEnumerator thisEvent) {
        mainEvents.Do(thisEvent);
    }

    public void DoIn(IEnumerator thisEvent, int delay) {
        mainEvents.DoIn(thisEvent, delay);
    }

    public void DoRepeating(IEnumerator thisEvent, int iterations, int delay, int interval) {
    ...
    public IEnumerable DoBlocking(IEnumerator thisEvent) {
        return mainEvents.DoBlocking(thisEvent);
    }

    // Do not use this unless you HAVE to
    // This will block all things on the callers EventLoop
    public IEnumerable<T> DoGet<T>(IEnumerator<T> thisEvent) {
        return mainEvents.DoGet(thisEvent);
    }
}